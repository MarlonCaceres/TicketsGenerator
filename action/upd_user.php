<?php	
	...
	if (empty($_POST['mod_name'])) {
		...
		}else if ($_POST['mod_rol']==""){
			$errors[] = "El usuario debe tener un rol";
		}else if ($_POST['mod_empresa']==""){
			$errors[] = "El usuario debe pertenecer a una Empresa";
		}else if (
		...
		$rol=$_POST['mod_rol'];
		$empresa=intval($_POST['mod_empresa']);
		$id=$_POST['mod_id'];

		$sql="UPDATE user SET name=\"$name\", email=\"$email\",is_active=$status, role=\"$rol\",Empresa=$empresa  WHERE id=$id";
